...
import type { Account, Transaction, Category, RecurringTransaction, Notification, Goal } from '../types';
import { TransactionType, NotificationType } from '../types';
import AlertBanner from './AlertBanner';
import AiInsights from './AiInsights';
import { getNextDueDate } from '../utils';
import WeeklySpendingStackedBarChart from './WeeklySpendingStackedBarChart';
...
const KpiCard: React.FC<{ title: string; value: number; change?: number | null; isCurrency?: boolean; icon: React.ReactNode }> = ({ title, value, change, isCurrency = true, icon }) => {
    ...
    return (
        <div className="bg-gradient-to-br from-indigo-50 to-indigo-100 p-7 rounded-2xl shadow-lg border border-indigo-200 flex items-center justify-between transition-all duration-300">
            <div>
                <p className="text-base text-indigo-600 font-semibold tracking-wide mb-1">{title}</p>
                <p className="text-4xl font-extrabold text-gray-900 mt-1 drop-shadow-sm">{formatValue(value)}</p>
                {hasChange && (
                    <div className={`mt-2 flex items-center gap-2 text-sm font-bold ${isPositive ? 'text-green-600' : isNegative ? 'text-red-600' : 'text-gray-500'}`}>
                        {isPositive ? '▲' : isNegative ? '▼' : ''}
                        <span>{formatValue(Math.abs(change))} ce mois-ci</span>
                    </div>
                )}
            </div>
            <div className="bg-indigo-200 text-indigo-700 p-3 rounded-xl shadow-md">
                {icon}
            </div>
        </div>
    );
...
const Dashboard: React.FC<DashboardProps> = ({ accounts, transactions, categories, recurringTransactions, notifications, setNotifications, goals, geminiApiKey }) => {
    ...
    // Déclaration du tableau dashboardBlocks juste avant le return
    const dashboardBlocks = {
        ...
        cashflow: <WeeklySpendingStackedBarChart transactions={transactions} categories={categories} />,
        donut: <SpendingDonutChart data={spendingBreakdown} />,
        soldes: (
            <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-200">
    ...
    return (
        <div className="w-full h-full px-2 md:px-6 xl:px-16 py-6 space-y-8">
            {/* Notifications */}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <KpiCard title="Patrimoine Net" value={netWorth} change={netWorthChange} icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-7 w-7" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M3 6l3 1m0 0l-3 9a5.002 5.002 0 006.001 0M6 7l3 9M6 7l6-2m6 2l3-1m-3 1l-3 9a5.002 5.002 0 006.001 0M18 7l3 9m-3-9l-6-2m0-2v2m0 16V5m0 16H9m3 0h3" /></svg>} />
                <KpiCard title="Flux de Trésorerie" value={monthlyCashFlow} icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-7 w-7" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M4 4v5h5M7 7l9 9M20 20v-5h-5M17 17l-9-9" /></svg>} />
                <KpiCard title="Dépenses du Mois" value={monthlyExpenses} icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-7 w-7" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" /></svg>} />
            </div>

            {/* LIGNE 2 : IA pleine largeur */}
            ...
            {/* LIGNE 3 : Analyse & Détails */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-start">
                {/* Colonne gauche (2/3) */}
                <div className="md:col-span-2">
                    <WeeklySpendingStackedBarChart transactions={transactions} categories={categories} />
                </div>
                {/* Colonne droite (1/3) */}
                <div className="flex flex-col gap-6">
                    {/* Soldes version compacte */}
                    <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200">
                        <h3 className="font-bold text-gray-800 text-sm mb-2">Soldes des Comptes</h3>
                        <ul className="space-y-2">
                            {accounts.map(acc => (
                                <li key={acc.id} className="flex justify-between items-center text-xs">
                                    <span className="text-gray-600">{acc.name}</span>
                                    <span className="font-semibold text-gray-800">{(acc.balanceHistory[0]?.amount ?? 0).toLocaleString('fr-FR', {style:'currency', currency:'EUR'})}</span>
                                </li>
                            ))}
                        </ul>
                        <div className="mt-2 pt-2 border-t border-gray-200 flex justify-between items-center font-bold text-gray-800 text-xs">
                            <span>Total</span>
                            <span>{totalBalance.toLocaleString('fr-FR', {style:'currency', currency:'EUR'})}</span>
                        </div>
                    </div>
                    {/* Widget à onglets Budgets / Échéances */}
                    <TabWidget budgets={dashboardBlocks.budgets} deadlines={dashboardBlocks.deadlines} />
                </div>
            </div>
        </div>
    );
